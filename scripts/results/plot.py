...
def plot_scaling_curve(
    ...
    for stage in ["pretrain", "sft", "grpo"]:
        ...
        ax.plot(
            ...
            label=stage, linewidth=2, markersize=8,
        )
        ...
        if ci_col in stage_data.columns:
            ...
            ax.fill_between(
                grouped.index, ci_low.values, ci_high.values,
                alpha=0.15, color=STAGE_COLORS.get(stage, "gray"),
            )
...
def plot_mixture_heatmap(
    ...
    pivot = subset.pivot_table(
        values=metric, index="model_depth",
        columns="pretrain_mixture", aggfunc="mean",
    )
    if pivot.empty:
        return
    ...
    try:
        ...
        sns.heatmap(
            ...
            vmin=0, vmax=max(0.5, pivot.max().max() * 1.2),
            linewidths=1, ax=ax,
        )
    except ImportError:
...
def plot_cost_efficiency(
    ...
    if frontier_x:
        ax.plot(
            frontier_x, frontier_y, "--",
            color="gray", alpha=0.5, label="Pareto frontier",
        )
...
def plot_rl_dynamics(
    ...
    Expects df with columns: rl_step, reward_mean, and optionally
    curriculum, gsm8k_pass1.
    """
    _apply_style()
    ...
    curricula = (
        df["curriculum"].unique() if "curriculum" in df.columns else ["default"]
    )
    colors = plt.cm.tab10(np.linspace(0, 1, max(len(curricula), 1)))

    for i, curr in enumerate(curricula):
        sub = (
            df[df["curriculum"] == curr].sort_values("rl_step")
            if "curriculum" in df.columns
            else df.sort_values("rl_step")
        )

        ax1.plot(
            sub["rl_step"], sub["reward_mean"],
            color=colors[i], label=f"{curr} reward", linewidth=2,
        )

        if "gsm8k_pass1" in sub.columns:
            ax2.plot(
                sub["rl_step"], sub["gsm8k_pass1"],
                color=colors[i], linestyle="--",
                label=f"{curr} GSM8K", linewidth=2,
            )
...
def plot_recipe_comparison(
    ...
    ax.set_ylabel(metric.replace("_", " ").title())
    ax.set_title(
        f"{dataset.upper()} — {group_col.replace('_', ' ').title()} Comparison"
    )
    ax.legend()
    plt.tight_layout()
